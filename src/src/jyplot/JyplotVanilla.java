...
/**
 * The only dependenecies this class has is to JFreeChart itself.  
 * That means that is will load quicker and the memory footprint
 * will be smaller.  However, it lacks advanced options such as
 * <p>
...
//		if(new File("/home/dion/storage/projects/javaprojects/log4j.parameters").exists())
//		{
...
//		}
		 ...
		 */
		// System.setProperty("java.awt.headless", Boolean.toString(enableJFrame));
	...
	public void subplot(int rows, int cols, int plotnumber)
	{
		 ...
		 */
		//		if(this.chartarray == null || plotnumber == 0 || rows != this.chartarray.length || cols != this.chartarray[0].length)
		//		{
		...
		//			 */
		//			if(this.chartarray.length == rows && this.chartarray[0].length == cols)
		//			{
...
//		}
		...
		//			renderer = (XYLineAndShapeRenderer)plot.getRendererForDataset(dataset);
		//			
		//			/*
...
//		if(color != null)
//			plot.getRendererForDataset(dataset).setSeriesPaint(dataset.getSeriesCount() - 1, color);
...
//			chart.getTitle().setFont(new Font("Serif", Font.PLAIN, 12));
	...
	public void title(String p, int fontsize)
	{
		...
		{
			chart.setTitle(p);
			chart.getTitle().setFont(new Font("SansSerif", Font.BOLD, fontsize));
		}
		else
		{
			System.out.println("title(): No chart created");
	...
	public void xlabel(String p)
	{
		if(chart != null)
		{
			...
			ValueAxis axis = chart.getXYPlot().getDomainAxisForDataset(currentDataSetIndex);
	...
	public void legend(String[] p)
	{
		...
		if(p == null)
		{
			...
			{
				this.getChart().getXYPlot().getRenderer(i).setSeriesVisibleInLegend(false);
			}
		...
		{
			logger.debug("chart.getXYPlot().getDataset().getClass() != BarXYDatasetClass");
		}
	...
	public void label(String p)
	{
		...
		try
		{
			...
			{
				((XYSeriesCollection) this.chart.getXYPlot().getDataset(latestDatasetIndex)).getSeries(this.chart.getXYPlot().getDataset(latestDatasetIndex).getSeriesCount() - 1).setKey(p);
				
				this.chart.getXYPlot().getRendererForDataset(this.chart.getXYPlot().getDataset(latestDatasetIndex)).setSeriesVisibleInLegend(true);
			}
			else
			{
				this.chart.getXYPlot().getRendererForDataset(this.chart.getXYPlot().getDataset(latestDatasetIndex)).setSeriesVisibleInLegend(latestDatasetIndex, false);
				this.chart.getXYPlot().getRendererForDataset(this.chart.getXYPlot().getDataset(latestDatasetIndex)).setSeriesVisibleInLegend(false);
				this.chart.getXYPlot().getRendererForDataset(this.chart.getXYPlot().getDataset(latestDatasetIndex)).setSeriesVisibleInLegend(false, true);
			}
		}
		catch (Exception e)
		{
	...
	public void linestyle(String[] p)
	{
		...
		for (int i = 0; i < this.dataset.getSeriesCount(); i++)
		{
			...
			if(p.length >= i)
			{
				...
				r1.setSeriesStroke(i, new BasicStroke(stroke.getLineWidth(), stroke.getEndCap(), stroke.getLineJoin(), stroke.getMiterLimit(), new float[]{12, 12}, 0));
			}
			else
				break;
		}
	 ...
	 * 
	 * @param p[]
	 *            gets an array of the axis boundries: p0 = lower x, p1 = upper
	 *            x p2 = lower y, p3 = upper y
	 */
	...
	public void xlim(double min, double max)
	{
		if(this.chart == null)
		{
			logger.error("No chart yet.");
			return;
		}
		try
		{
			ValueAxis XAxis = this.chart.getXYPlot().getDomainAxis();
//			int currentDatasetIndex = this.chart.getXYPlot().getDatasetCount();
//			
//			ValueAxis XAxis = this.chart.getXYPlot().getDomainAxisForDataset(currentDatasetIndex);
...
//			int currentDatasetIndex = this.chart.getXYPlot().getDatasetCount();
//			
//			ValueAxis YAxis = this.chart.getXYPlot().getRangeAxisForDataset(currentDatasetIndex);
...
//		j.errorbar(new double[]{1, 2, 3}, new double[]{1, 2, 1}, null, new double[]{0.5, 0.5, 0.4}, j.getColor("blue"), true);

		//		 j.plot(null, y2, new Color(255,255,0), "y2", null,  false,false, "y2", true);
		//		 j.plot(null, y1, new Color(0,255,0), "y1", null,   false,false, "y1", true);
		//		 j.subplot(132);
		//		 j.plot(null, x1, new Color(0,255,255), "x1", null,   false,false, "x1", true);
		//		 j.subplot(133);
		//		 j.difference(null, x1, y1, new Color(0,255,255,25), false);

		//		  plot(double[] x, double[] y, Color color, double[] yErrorBars, boolean multipleAxes, boolean newRange, String rangeLabel)
...
//		j.title("foobar");
		...
		//		j.savefig(System.getProperty("user.home") + "/tmp/test.png", 200, 200);
...
//		j.savefig("/tmp/delete.jpg", 100, 100);
	...
	public BarXYDataset bar(double[] left, double[] height, double[] width, double[] bottom, Color color)
	{
		...
		if(this.chart == null)
		{
			...
			this.chart = ChartFactory.createXYBarChart(null, null, false, null, dataset, PlotOrientation.VERTICAL, false, false, false);
			...
			//			this.chartarray[this.currentRow][this.currentCol] = this.chart;// Put
			// in
			// the
			// array
			// for
		 ...
		 */
		{
			...
			{
				if(this.chart.getXYPlot().getDataset(i) != null && this.chart.getXYPlot().getDataset(i).getClass().getName().matches(".*BarXYDataset"))
				{
		...
		BarXYDataset dataset = (BarXYDataset) this.chart.getXYPlot().getDataset(datasetIndex);
		/*
		 * Make sure the dataset has the right renderer
		 */
		if(this.chart.getXYPlot().getRenderer(datasetIndex) != null && this.chart.getXYPlot().getRenderer(datasetIndex).getClass() == XYBarRenderer.class) // .getName().matches(".*XYBarRenderer")
		{
			...
			// ((XYBarRenderer) this.chart.getXYPlot().getRenderer(datasetIndex)).setSeriesVisibleInLegend(true);
		}
		else
		/* The render is */
		{
	...
	//			CustomSymbolAxis symbolaxis = new CustomSymbolAxis(xNumberAxis.getLabel(), ticklabels, new double[] {-1, 1, 2});
	////			symbolaxis.
		...
		for (int row = 0; row < this.rows; row++)
		{
			for (int col = 0; col < this.cols; col++)
			{
				...
				if(this.chart != null)
				{
					...
					//					
					chart.draw(g, new java.awt.Rectangle(chartX, chartY, chartwidth, chartheight), new Point(chartX, chartY), info);
...
//			AbstractRenderer r1 = (AbstractRenderer) plot.getRenderer(0);
//			int seriesindex = this.chart.getXYPlot().getDataset().getSeriesCount() - 1;
//			if(seriesindex >= 0)
...
public Color getColor(String s)
	{
		...
		{	
			...
			logger.warn("No color matching " + s);
...
//		XYSeriesCollection dataset = getXYDataset();
		...
		for (int i = 0; i < y1.length; i++)
		{
			if(!Double.isNaN(x[i]) && !Double.isNaN(y1[i]) && !Double.isNaN(y2[i]))
			{
		...
		XYDifferenceRenderer differenceRenderer = new XYDifferenceRenderer(color, color, false);
	...
	public XYDataset hist(double[] data, int bins, String dataname, Color color)//, double min, double max)
	{
	 ...
	 * Preserves compatability with matplotlib.
	 *
	 */
	public void clf()
	{
	...
	protected Color getMostRecentColor()
	{
		try
		{
			...
			//			XYLineAndShapeRenderer renderer = (XYLineAndShapeRenderer)this.chart.getXYPlot().getRenderer(datasetIndex);

			System.out.println(this.chart.getXYPlot().getRendererForDataset(dataset).getClass().getName());
			return (Color) this.chart.getXYPlot().getRendererForDataset(dataset).getSeriesPaint(dataset.getSeriesCount() - 1);
		...
		//			XYPlot plot = (XYPlot) chart.getPlot();
		//			AbstractRenderer r1 = (AbstractRenderer) plot.getRenderer(0);
		//			int seriesindex = this.chart.getXYPlot().getDataset(this.chart.getXYPlot().getDatasetCount()).getSeriesCount() - 1;
		//			System.out.println("seriesindex:" + seriesindex);
		//			if(seriesindex >= 0)
		//			{
		//				System.out.println( r1.getSeriesPaint(seriesindex).getClass().getName() );
		//				
...
/**
	...
	protected boolean isXYPlot()
	{
		...
		{
			logger.error("chart.getPlot() is null");
			return false;
		}
		return true;
	}
	...
	protected void setSeriesColor(Color color)
	{
		...
		if(plot == null)
		{
			logger.error("setSeriesColor() but plot is null");
			return;
		}
		
		if(color != null && plot.getDatasetCount() > 0)
		{
			plot.getRendererForDataset(plot.getDataset( plot.getDatasetCount() - 1)).setSeriesPaint(plot.getDataset( plot.getDatasetCount() - 1).getSeriesCount() - 1, color);
		}
